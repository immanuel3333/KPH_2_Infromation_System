    ...
    <div class="card" style="padding-top:100px;">
        ...
        <div>
		 ...
		 <script type="text/javascript">
			...
			function klikhome(){
                $.get("{{url('identitas2')}}", function(data) {
                $("#konten").html(data);
                });
            }
            function kepalakph(){
                $.get("{{url('kepalakph2')}}", function(data2) {
                $("#konten").html(data2);
                });
            }
            function fasilitas(){
                $.get("{{url('fasilitas2')}}", function(data3) {
                $("#konten").html(data3);
                });
            }
            function lembaga(){
                $.get("{{url('lembaga2')}}", function(data4) {
                $("#konten").html(data4);
                });
            }
            function rphjp(){
                $.get("{{url('rphjp2')}}", function(data5) {
                $("#konten").html(data5);
                });
            }

            $('.konten').load("{{url('identitas2')}}");
